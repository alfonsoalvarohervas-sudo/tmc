...
| **M4 convert behavior `#ifdef`s to runtime** | IN PROGRESS — **~75 function-level sites converted** to `if(REGION_IS_*)` across ~40 entity/object/npc/item/menu files (via the `m4-convert-behavior-wave1` workflow, build + all-3-boot verified after each wave). Latest wave `b41c0333`. **TRAP (found + fixed in `areaMetadata.c`/`gameUtils.c`):** flattening a per-region *data table* while its *consumers* stay region-split silently breaks the non-baseline region — the table's flag bits and the consumers' equality constants must agree. Pick one: per-region table, OR single-baseline table + region-agnostic consumers. Never half-and-half. REMAINING: (a) **data-table pattern** (`objectDefinitions.c` 51, `sound.c` gSongTable, `projectile.c`, `itemMetaData.c`, `*Definitions.c`, `data/*.c`, sprite-index `#define`s) — needs per-region data, not control flow; (b) **exclusive-function dispatch** (whole-function `#ifdef` defs — un-guard + runtime-select, addresses from the maps); (c) **special cases** (`case`-label guards, brace-splits, `&&`-chain operands like `collision.c` knockback, 3-way `#elif`); (d) **core/save files** deferred (`save.c`, `fileselect.c`, `main.c`, `interrupts.c`, `common.c`, `game.c`, `script.c`, `player.c`). DEMO_* sites left alone. |
