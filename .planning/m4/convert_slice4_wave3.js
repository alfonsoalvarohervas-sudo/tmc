export const meta = {
  name: 'm4-slice4-convert-wave3',
  description: 'M4 slice-4 wave 3: remaining per-region sites, chunked to avoid rate limits',
  phases: [ { title: 'Convert', detail: 'chunks of 5 agents (<=5 concurrent) to dodge rate limits' } ],
}
...
for (let i = 0; i < FILES.length; i += CHUNK) {
  ...
  log(`chunk ${i/CHUNK+1}: ${batch.map(b=>b[0]).join(', ')}`)
  const out = await parallel(batch.map(f => () => runFile(f)))
  results.push(...out)
}
log(`Wave3: converted ${results.filter(r=>r&&r.applied).length}/${FILES.length}; ` +
