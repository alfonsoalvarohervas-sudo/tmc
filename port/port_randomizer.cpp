...
extern "C" PortRandomizerStatus Port_Randomizer_RollSeed(
    ...
    if (!PathExists(input_rom_path)) {
        report("input ROM not found at the given path");
        return PORT_RANDO_INPUT_ROM_MISSING;
    }

    /* Read the ROM region from header bytes 0xAC..0xAF:
     *   BZMP  → EU  (use the default logic + EU patches shipped in the rando)
     *   BZME  → USA (use the USA logic + USA-translated patches generated by
     *                tools/randomizer_usa/build_usa_patches.py)
     *   BZMJ  → JP  (no support yet — treat as EU and let CRC check fail)
     * Anything else is treated as EU so the CLI emits its own clear error. */
    char region[5] = {0};
    {
        std::ifstream rom_in(input_rom_path, std::ios::binary);
        if (rom_in.is_open()) {
            rom_in.seekg(0xAC);
            rom_in.read(region, 4);
            region[4] = '\0';
        }
    }
    const bool is_usa = (std::strcmp(region, "BZME") == 0);
    ...
    /* Locate the USA artifacts (logic file + patches tree) relative to
     * the CLI binary. They sit next to MinishCapRandomizerCLI under
     * <exe>/randomizer/ since the dist staging in build.py copies the
     * whole randomizer output tree. */
    std::filesystem::path rando_dir = std::filesystem::path(cli_path).parent_path();
    std::filesystem::path usa_logic = rando_dir / "default_usa.logic";
    std::filesystem::path usa_patch = rando_dir / "Patches_USA" / "ROM Buildfile.event";

    if (is_usa) {
        if (!PathExists(usa_logic) || !PathExists(usa_patch)) {
            report("input is a USA ROM but USA logic/patches are missing — "
                   "build them via `python3 tools/randomizer_usa/build_usa_patches.py` "
                   "or supply an EU ROM");
            return PORT_RANDO_RUN_FAILED;
        }
    }
    ...
    const std::filesystem::path cmd_file = exeDir / "tmc_rando_commands.txt";
    {
        ...
        if (is_usa) {
            /* Override logic + patches BEFORE ChangeSeed so the CRC check
             * inside Randomize uses the USA logic file. The CLI's
             * LoadLogic / LoadPatch commands cache paths that
             * ApplyPatch then consumes. */
            out << "LoadLogic " << usa_logic.string() << "\n";
            out << "LoadPatch " << usa_patch.string() << "\n";
        }
