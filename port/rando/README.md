...
- **scripted runtime keys**: `rando_keymap.h/.c` reserves bit 31 for stable
  ...
  prize, the Deepwood/CoF/Droplets/Palace dungeon prizes, and Business Scrub
  item sales by default (36 locations at stock settings). With `GORON_5` +
  `VANILLA_BLUE_FUSIONS` + `VANILLA_RED_FUSIONS` + `BIGGORON_NORMAL` +
  `CUCCO_10`, 65/66 scripted locations bind in the real-file diagnostic;
...
- per-location keyed hooks are still missing for the remaining NPC-script and
  ...
  pedestal items, direct fusion-item grants / similar one-off scripts). Those
...
set) probes 242 keyed locations at default settings (161 chests + 45 ground
items from `rando_keymap.c` + 36 scripted rewards from the high-bit runtime
namespace); 156 chest keys match real engine chest TileEntities (all 156
key-resolve through the runtime `Rando_RoomChestIndex` path) and 241
...
- 156/176 item symbols map to native engine ids (gear, elements, scrolls,
  ...
  vanilla reward in place. At default settings 241/242 keyed locations
